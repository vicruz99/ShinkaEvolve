...
def evaluation_function(data_db, signal):
    ...
    ### AQUI PODIA SER FIXE ADICIONAR BASELINE ALEATÒRIA - PARA PERMITIR A COMPARAÇÂO ENTRE EMPRESAS DIFERENTES
...
def aggregate_metrics_fn(results: List[Any], all_run_kwargs: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        "combined_score": np.mean(list(companies_score.values())) if companies_score else 0.0,
        "indivisual_companie_scores": companies_score,
    }
...
def main(program_path: str, results_dir: str):
    ...
    NUMBER_OF_RUNS = 100                                          # How many times to run the code  - IT WOULD BE COOL FOR THIS TO BE CONFIGURABLE FROM THE OUTSIDE, BUT FOR NOW LET'S KEEP IT FIXED TO A HIGH NUMBER TO ENCOURAGE ROBUSTNESS AND AVOID OVERFITTING TO THE TEST SET.
